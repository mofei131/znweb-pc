    ...
    <el-table v-loading="loading" :data="bidApplyList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button v-if="!scope.row.state || scope.row.state == '4'"
            size="mini"
            type="text"
            icon="el-icon-edit"
          ...
          <el-col :span="12">
            <el-form-item label="单价模式" prop="unitPriceMode">
              <el-radio-group v-model="form.unitPriceMode">
                <el-radio v-for="(item,index) in unitPriceModeOptions" :key="index" :label="item.key">{{item.label}}</el-radio>
              </el-radio-group>
        </el-form-item>
          </el-col>
          <el-col :span="12">
...
<script>
import { listBidApply, getBidApply, delBidApply, addBidApply, updateBidApply, getStList } from "@/api/project/bidApply";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listBidApply(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除投标申请编号为"' + bidIds + '"的数据项?', "警告", {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('project/bidApply/export', {
        ...this.queryParams
