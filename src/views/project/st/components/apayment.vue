    ...
    <div class="app-container">
        <!-- <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
            ...
            <el-form-item label="项目编号" prop="stNo">
                <el-input v-model="queryParams.stNo" placeholder="请输入项目编号" clearable size="small"
        ...
        <el-table v-loading="loading" :data="apaymentList" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="总量(吨)" align="center" prop="grns">
            ...
            <el-table-column label="总量(吨)" align="center" prop="grys">
                <template slot-scope="scope">
                    {{
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="140px">
                <div v-if="isLook != 4">
                    <el-row>
                        <el-col :span="12">
                            <el-form-item label="项目" prop="stId">
                                <el-select filterable value-key="stId" @change="changeSt" v-model="form.stId"
                                    placeholder="请选择项目" style="width: 100%">
                                    <el-option v-for="obj in stOptions" :key="obj.stId" :label="obj.name" :value="obj">
                                    </el-option>
                                </el-select>
                            </el-form-item>
                        </el-col>
                    ...
                    <el-row>
                        <el-col :span="12">
                            <el-form-item label="预付批次" prop="away">
                                <el-select v-model="form.away" @change="changeAway" placeholder="请选择预付批次"
                                    style="width: 100%">
                                    <el-option v-for="obj in awaysOptions" :key="obj.key" :label="obj.label"
                                        :value="obj.key"></el-option>
                                </el-select>
                            </el-form-item>
                        </el-col>
                        <div v-if="form.away != '提前付款'">
                            <el-col :span="12">
                                <el-form-item label="预付方式" prop="type">
                                    <el-select v-model="form.type" @change="jsdj" placeholder="请选择预付方式"
                                        style="width: 100%">
                                        <el-option v-for="obj in typesOptions" :key="obj.key" :label="obj.label"
                                            :value="obj.key"></el-option>
                                    </el-select>
                                </el-form-item>
                            </el-col>
                    ...
                    ">
                        ...
                        <el-popover placement="bottom-start" width="100%" @selection-change="grnSelectionChange"
                            ...
                            <el-button size="small" type="primary" slot="reference"
                                style="margin-bottom: 30px;margin-left:50px">选择随车数质量</el-button>
                            ...
                            <div style="margin-top: 20px">
                                <el-button type="primary" style="float: right" @click="toggleSelection()">确认选择
                                </el-button>
                            </div>
                        </el-popover>
                    </div>
                    ...
                    ">
                        ...
                        <el-popover placement="bottom-start" width="100%" @selection-change="grnSelectionChange"
                            v-model="visible" popper-class="area_popper">
                            <el-button size="small" type="primary" slot="reference"
                                style="margin-bottom: 30px;margin-left:50px">选择到厂数质量</el-button>
                    ...
                    <div v-if="form.away != '提前付款'">
                        <div style="margin-bottom: 30px">
                            <el-table ref="singleTable" :data="tableselData" style="width: 100%">
                                ...
                                <el-table-column v-if="form.away == '首次'" property="grnNumber" label="重量（吨）"
                                    width="120">
                                    <template slot-scope="scope">
                                        {{
                                                Number(scope.row.grnNumber)
                                ...
                                <el-table-column v-if="form.away == '二次'" property="grnNumber" label="重量（吨）"
                                    width="120">
                                    <template slot-scope="scope">
                                        {{
                                                Number(scope.row.grnNumber)
                                ...
                                <el-table-column v-if="form.away == '首次'" property="grnRz" label="热值（kcal）"
                                    width="120">
                                </el-table-column>
                                <el-table-column v-if="form.away == '二次'" property="gryRz" label="热值（kcal）"
                                    width="120">
                                </el-table-column>
                                <el-table-column property="transportType" label="运输方式" width="90">
                                </el-table-column>
                                ...
                                <el-table-column v-if="form.away == '首次'" property="deliveryTime" label="发货日期"
                                    width="120">
                                </el-table-column>
                                <el-table-column v-if="form.away == '二次' && isLook != 3" property="okTime" label="到货日期"
                                    width="120">
                                </el-table-column>
                                <el-table-column v-if="form.away == '二次' && isLook == 3" property="deliveryTime"
                                    label="到货日期" width="120">
                                </el-table-column>
                        ...
                        <el-row style="margin-bottom:20px">
                            ...
                            <el-col :span="24" style="margin-top:10px;margin-left:60px">
                                ...
                                <el-button size="small" style="margin-left: 10px" type="primary" @click="jsjc"
                        ...
                        <el-row>
                            ...
                            <el-col :span="12">
                                <el-form-item label="预付至" prop="ato">
                                    <el-select v-model="form.ato" @change="atochange" placeholder="请选择预付批次"
                                        style="width: 100%">
                                        <el-option v-for="obj in atosOptions" :key="obj.key" :label="obj.label"
                                            :value="obj.key"></el-option>
                                    </el-select>
                                </el-form-item>
                            </el-col>
                        ...
                        <el-col :span="12">
                            <el-form-item label="附件" prop="file">
                                <el-upload class="upload-demo" :action="url" :headers="headers"
                                    ...
                                    :on-error="uploadError" :before-remove="beforeRemove" multiple :limit="10"
                ...
                <div v-if="isLook == '4'">
                    ...
                    <el-row>
                        ...
                        <el-col :span="12">
                            <el-form-item label="付款日期" prop="payTime">
                                <el-date-picker clearable size="small" style="width: 100%" v-model="form.payTime"
                                    type="date" value-format="yyyy-MM-dd" placeholder="选择付款日期">
                                </el-date-picker>
                            </el-form-item>
                        </el-col>
                    </el-row>
                ...
                <div class="search-title-content">
                    <div style="padding: 0 0 15px">
                        <el-row type="flex" justify="space-between">
                            <el-col :span="4"><span style="font-weight: bold; font-size: 16px"
                                    v-text="printData.printType"></span></el-col>
