...
<!--      <el-table-column label="项目id" align="center" prop="stId" />-->
      ...
      <el-table-column label="类型"  align="center" prop="xmState" :formatter="xmStateFormat" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="70%" append-to-body>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="180px">
        ...
        <div style="margin-top: 50px;" v-if="active=='1' && isLook!=4">
          <el-row>
            <el-col :span="12">
              <el-form-item label="自助金融方" prop="shType">
              ...
              <el-col :span="12">
                <div v-if="form.shChanneltype=='1'">
                  <el-form-item label="下游渠道年限" prop="shChannelyear">
                    <el-input v-model="form.shChannelyear" placeholder="请输入下游渠道年限"  :rules="form.shType == '1' ? rules.shChannelyear : [{ required: false }]"/>
                  </el-form-item>
                </div>
              </el-col>
            </el-row>
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="备注说明" prop="node">
                <el-input type="textarea" :row="4" v-model="form.node" placeholder="请输入备注说明" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        shChannelyear: [
          { required: true, message: "渠道年限不能为空", trigger: "blur" }
        ],
        shSettlement: [
          { required: true, message: "请选择结算比例", trigger: "blur" }
        ],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listStUpdate(this.queryParams).then(response => {
    ...
    handleLook(row) {
      ...
      this.active=0;
      const stId = row.stId
      const stupdateId = row.stupdateId
      this.$router.push({name: 'lookUpdate', query:{stId:stId+"",stupdateId:stupdateId}})
      // this.$router.push({path:"/st/lookUpdate/" + stId,params:{"stId":stId,"xmNode":row.xmNode}});
      // getSt(stId).then(response => {
      //   this.form = response.data;
      //   this.fileList = response.data.fileList
      //
