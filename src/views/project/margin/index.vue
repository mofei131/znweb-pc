    ...
    <el-table v-loading="loading" :data="marginList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          >查看</el-button>
          <el-button
            v-if="scope.row.zjState=='1' && scope.row.type=='上游'"
            size="mini"
            type="text"
            icon="el-icon-edit"
          ...
          >退还</el-button>
          <el-button
            v-if="scope.row.zjState=='1' && scope.row.type=='下游'"
            size="mini"
            type="text"
            icon="el-icon-edit"
            @click="handleBack(scope.row)"
...
<script>
import { listMargin, getMargin, delMargin, addMargin, updateMargin,getStList,getContractListAll,getTerminalListAll,getSupplierListAll} from "@/api/project/margin";
...
export default {
  ...
  methods: {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('project/margin/export', {
        ...this.queryParams
      }, `project_margin.xlsx`)
    },
