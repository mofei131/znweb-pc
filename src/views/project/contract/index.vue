          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="终端客户" prop="terminalId">
                <el-select   filterable value-key="terminalId" @change="changeTerinal"  v-model="form.terminalId" placeholder="请选择终端客户" style="width: 100%;">
                  <el-option
...
<script>
import { listContract, getContract, delContract, addContract, updateContract, getStList, getSupplierList, getTerminalList} from "@/api/project/contract";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        type: [
          { required: true, message: "情选择合同类型", trigger: "blur" }
        ],
        signingTime: [
        ...
        transportUnit: [
          { required: true, message: "运输单位/服务单位不能为空", trigger: "blur" }
        ],
        transportType: [
  ...
  methods: {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('project/contract/export', {
        ...this.queryParams
